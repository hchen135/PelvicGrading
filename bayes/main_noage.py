...
for i in range(args.num_fold):
    ...
    bn.fit_node_states(train.drop(columns=["name","age"]))
    ...
    refined = refine_predictions(test_high_conf, test_low_conf, ie, args.threshold, 1, no_age=True)
...
all_data.extend([ALL_GT_result['ROT_acc'],ALL_High_result['ROT_acc'],ALL_Low_result['ROT_acc'],ALL_Refine_result['ROT_acc'],ALL_GT_result['TRA_acc'],ALL_High_result['TRA_acc'],ALL_Low_result['TRA_acc'],ALL_Refine_result['TRA_acc']])
all_data.extend([ALL_GT_result['ROT_kappa'],ALL_High_result['ROT_kappa'],ALL_Low_result['ROT_kappa'],ALL_Refine_result['ROT_kappa'],ALL_GT_result['TRA_kappa'],ALL_High_result['TRA_kappa'],ALL_Low_result['TRA_kappa'],ALL_Refine_result['TRA_kappa']])
all_data.extend([ALL_GT_result['ROT_auc'],ALL_High_result['ROT_auc'],ALL_Low_result['ROT_auc'],ALL_Refine_result['ROT_auc'],ALL_GT_result['TRA_auc'],ALL_High_result['TRA_auc'],ALL_Low_result['TRA_auc'],ALL_Refine_result['TRA_auc']])

# Save results to csv files
if not os.path.exists(os.path.join(result_csv_name)):
    columns = ["High_threshold","Low_threhsold","Threshold"]
    columns.extend(["GT_ROT_acc","High_ROT_acc","Low_ROT_acc","Refine_ROT_acc","GT_TRA_acc","High_TRA_acc","Low_TRA_acc","Refine_TRA_acc"])
    columns.extend(["GT_ROT_kappa","High_ROT_kappa","Low_ROT_kappa","Refine_ROT_kappa","GT_TRA_kappa","High_TRA_kappa","Low_TRA_kappa","Refine_TRA_kappa"])
    columns.extend(["GT_ROT_auc","High_ROT_auc","Low_ROT_auc","Refine_ROT_auc","GT_TRA_auc","High_TRA_auc","Low_TRA_auc","Refine_TRA_auc"])
